...
districts <- readr::read_csv(
  ...
  col_types = sprintf("ciiccccci%s", paste0(rep("d", 75), collapse = ""))
)
```
...
districts_analysis <- districts |> 
  ...
  mutate(
    cir = 100000 * new_cases_tbpfp / population_permanent,
    subregion = factor(
      subregion, 
...
districts_analysis |> 
  group_by(subregion) |> 
  summarise(new_cases_tbpfp = sum(new_cases_tbpfp), .groups = "drop") |> 
  mutate(prop_cases = 100 * new_cases_tbpfp / sum(new_cases_tbpfp))
```
...
subregion_cir <- districts_analysis |> 
  ...
  summarise(
    ...
    new_cases_tbpfp = sum(new_cases_tbpfp),
    cir_mean = mean(cir),
    .groups = "drop"
  ) |>
  mutate(
    prop_cases = 100 * new_cases_tbpfp / sum(new_cases_tbpfp),
    cir = 100000 * new_cases_tbpfp / population
  )
subregion_cir
```
...
work_subregions_adjacency <- igraph::as_adjacency_matrix(
   graphs_subregions$graph_raw[[3]], attr = "weight", sparse = FALSE
  )
...
total_subregions_adjacency <- igraph::as_adjacency_matrix(
   graphs_subregions$graph_raw[[2]], attr = "weight", sparse = FALSE
  )
...
```{r}
sum(districts_analysis$new_cases_tbpfp)
```
...
districts_analysis |> 
  arrange(cir) |> 
  select(district, ubigeo, district_short, cir, subregion)
```
...
```{r}
plot_scatter(
  districts_analysis, total_centrality_closeness_out, cir,
  "Out-strength centrality", "Incidence rate"
)
```
...
```{r}
districts_analysis |> 
  arrange(desc(total_centrality_closeness_out))
```

```{r}
total_centrality_closeness_out_biscale <- bi_class(
  districts_analysis_polygons, x = total_centrality_closeness_out, y = cir, 
  style = "jenks", dim = 3
)
```

```{r}
total_centrality_closeness_out_biscale_map <- total_centrality_closeness_out_biscale |> 
  ggplot() +
  geom_sf(
    aes(fill = bi_class), color = "white", 
    size = 0.05, show.legend = FALSE
...
total_centrality_closeness_out_biscale_map
```
...
```{r}
total_centrality_closeness_out_biscale_plot <- cowplot::ggdraw() +
  cowplot::draw_plot(total_centrality_closeness_out_biscale_map, 0, 0, 1, 1) +
  cowplot::draw_plot(biscale_legend, 0.13, 0.1, 0.25, 0.25) +
  theme(text = element_text(size = 20)) 

total_centrality_closeness_out_biscale_plot
```
...
glm_base_formulas_list <- list(
  "poverty-co" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + monetary_poverty + co,
  "poverty-no2" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + monetary_poverty + no2,
  "poverty-pm25" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + monetary_poverty + pm25,
  "poverty-o3" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + monetary_poverty + o3,
  "nonmonetary-co" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + nonmonetary_poverty + co,
  "nonmonetary-no2" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + nonmonetary_poverty + no2,
  "nonmonetary-pm25" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + nonmonetary_poverty + pm25,
  "nonmonetary-o3" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + nonmonetary_poverty + o3
)
```
...
gam_base_formulas_list <- list(
  "poverty-co" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(monetary_poverty) + s(co),
  "poverty-no2" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(monetary_poverty) + 
    s(no2),
  "poverty-pm25" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(monetary_poverty) + 
    s(pm25),
  "poverty-o3" = 
    ...
    s(o3),
  "nonmonetary-co" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(nonmonetary_poverty) + s(co),
  "nonmonetary-no2" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(nonmonetary_poverty) + s(no2),
  "nonmonetary-pm25" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(nonmonetary_poverty) + s(pm25),
  "nonmonetary-o3" = 
    new_cases_tbpfp ~ offset(log(population_permanent)) + s(nonmonetary_poverty) + s(o3)
)
```
...
study_poverty_plot <- gam_study_performance_fit_est |> 
  ...
  filter(smooth == "s(nonmonetary_poverty)") |> 
  ggplot() +
  geom_line(aes(nonmonetary_poverty, est), linewidth = 1) +
  geom_ribbon(
    aes(ymin = lower_ci, ymax = upper_ci, x = nonmonetary_poverty), alpha = 0.4,
    fill = "steelblue3"
  ) +
  geom_rug(
    aes(nonmonetary_poverty), data = districts_model, sides = "b", length = grid::unit(0.02, "npc")
  ) +
...
study_centrality_plot <- gam_study_performance_fit_est |> 
  mutate(across(c(est, lower_ci, upper_ci), exp)) |> 
  filter(smooth == "s(study_centrality_strength_all)") |> 
  ggplot() +
  geom_line(aes(study_centrality_strength_all, est), linewidth = 1) +
  geom_ribbon(
    aes(ymin = lower_ci, ymax = upper_ci, x = study_centrality_strength_all), alpha = 0.4,
    fill = "steelblue3"
  ) +
  geom_rug(
    aes(study_centrality_strength_all), data = districts_model, sides = "b", length = grid::unit(0.02, "npc")
  ) +
