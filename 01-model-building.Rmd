...
```{r}
fit <- gam(
  new_cases ~ offset(log(population_permanent)) + s(idh) + s(monetary_poverty) +
    s(life_expectancy) + s(complete_secondary_edu) + s(years_edu),
  data = dataset, select = TRUE, method = "REML"
)
summary(fit)
```
...
```{r}
x <- dataset %>% 
  dplyr::select(-c(population_permanent, new_cases, intra_mob:hub)) %>% 
  as.matrix()
```

```{r}
offset <- log(dataset$population_permanent)
```

```{r}
y <- pull(dataset, new_cases)
```


```{r}
fit <- glmnet(x, y, family = "poisson", offset = offset)
summary(fit)
```
...
```{r}
cvfit <- cv.glmnet(x, y, offset = offset)
```
...
```{r}
# Función para calcular el error de devianze
```
