...
tb_lima_raw %>% 
  filter(YEAR == 2017) %>% 
  select(
    ubigeo_eess, poblacion, total_casos, max_temperature:delta_temperature
  ) %>% 
...
```{r}
demo_filename <- "per-demographics-2017.csv" 
demo_path <- fs::path(raw_path, demo_filename)
demo_raw <- read_csv(demo_path, col_select = c(ubigeo, nib_hogar:monetary_poverty))
```

```{r}
mob_tb_lima <- mob_tb_lima %>% left_join(demo_raw, by = c("ubigeo_eess" = "ubigeo"))
```
