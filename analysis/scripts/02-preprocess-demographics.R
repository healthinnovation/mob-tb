...
demographics_filename <- "district-demographics.csv"
demographics_filepath <- path(raw_path, demographics_infilename)
demographics_raw <- read_csv(
  demographics_filepath, col_types = paste(c("cccc", rep("d", 14)), collapse = "")
)

population_filename <- "district-population-census-2017.csv"
population_filepath <- path(raw_path, population_infilename)
population <- read_csv(
  population_filepath, col_select = c("ubigeo", "population"), col_types = "cd"
)

demographics_full <- demographics_raw %>%
  inner_join(population, by = "ubigeo") %>%
  select(ubigeo:district, population, everything()) %>%
  mutate(
...
demographics <- demographics_full %>%
  ...
  select(
    ubigeo:monetary_poverty, life_expectancy = life_expectancy_norm,
    complete_secondary_edu, years_edu = years_edu_norm,
...
write_csv(demographics, nodes_filepath, na = "")
