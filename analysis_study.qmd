...
centralities_rank <- centralities |> 
  ...
  mutate(
    ...
    across(
      contains(c("strength", "close", "authority", "hub", "pagerank")), 
...
```{r}
model_1 <- model_results |> arrange(aic) |> filter(type == "Study", model_mode == "Adjusted")
```
...
```{r}
model_2 <- model_results |> arrange(aic) |> filter(type == "Study", model_mode == "Unadjusted")
```
...
```{r}
performance::check_model(model_1[[1]])
```
